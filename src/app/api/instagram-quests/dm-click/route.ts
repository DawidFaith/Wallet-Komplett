...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    // Ablauf prüfen
    if (new Date(verif.expiresAt) < new Date()) {
      return new NextResponse(buildPage('Link abgelaufen', 'Dieser Verifizierungslink ist abgelaufen. Bitte starte die Quest neu.', false), {
        headers: { 'Content-Type': 'text/html; charset=utf-8' },
      });
    }

    // Baseline Shares via MAKE_INSTAGRAM_LIKE_WEBHOOK_URL (identisch zu like-verify)
    let baselineShares = verif.baselineShares;
    if (!verif.clickVerified) {
      const makeWebhookUrl = process.env.MAKE_INSTAGRAM_LIKE_WEBHOOK_URL;
      const quest = await loadQuestDetail(verif.questId);
      if (makeWebhookUrl && quest?.videoId) {
        baselineShares = await fetchBaselineShares(makeWebhookUrl, quest.videoId);
      }
      await markInstagramDmClickedByHandle(handle, baselineShares);
    }

    const appUrl = process.env.NEXT_PUBLIC_APP_URL ?? 'https://wallet-komplett.vercel.app';
    const returnUrl = `${appUrl}/?tab=quests&dm_quest=${verif.questId}&step=2`;

    return new NextResponse(buildPage(
      '✅ Klick bestätigt!',
      `Teil 1 erfolgreich! Gehe jetzt zurück zur App und teile den Beitrag in deiner Story für Teil 2.`,
      true,
      returnUrl,
    ), {
      headers: { 'Content-Type': 'text/html; charset=utf-8' },
