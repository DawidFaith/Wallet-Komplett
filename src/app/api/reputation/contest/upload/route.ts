...
/** POST /api/reputation/contest/upload — Vercel Blob client-side upload für Contest-Banner */
  ...
  try {
    const jsonResponse = await handleUpload({
      ...
      onBeforeGenerateToken: async (_pathname, clientPayload) => {
        ...
        if (!rows.length || !rows[0].is_artist) throw new Error('Nur Artists dürfen Contest-Banner hochladen');
        return {
