/**
 * POST /api/tiktok-quests/save-verify
 *
 * Verifiziert einen TikTok Save-Quest.
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (quest.platform !== 'tiktok' || quest.type !== 'save') {
    ...
    if (action === 'check') {
      ...
      if (!saveVerified) {
        return NextResponse.json({
          ...
          notYet: true,
          saveVerified: false,
          message: 'Speichern noch nicht erkannt. Speichere das Video und prüfe erneut.',
      ...
      const completion: QuestCompletion = {
        ...
        platform: 'tiktok',
        commentId: `tiktok-save-${normalized}-${questId}`,
        commentText: 'Save:true',
      ...
      await savePendingReward({
        ...
        reason: `TikTok Save Quest: ${quest.videoTitle}`,
        questId,
        createdAt: now,
      });
      ...
      return NextResponse.json({
        ...
        message: `Gespeichert verifiziert! Du erhältst ${quest.rewardAmount + levelBonus + creditBonus} DFAITH.`,
      });
    }

  } catch (err) {
    const message = err instanceof Error ? err.message : String(err);
    console.error('[save-verify]', action, message);
