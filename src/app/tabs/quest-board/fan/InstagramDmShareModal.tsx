...
function StepBadge({ num, label, done, active }: { num: number; label: string; done: boolean; active: boolean }) {
  ...
  useEffect(() => {
    ...
    fetch('/api/instagram-quests/dm-share', {
      ...
      .then((data) => {
        if (data.started && !data.expired) {
          setExpiresAt(data.expiresAt ?? null);
          if (data.clickVerified && !data.storyVerified) {
            setStep('part1_done');
          } else if (!data.clickVerified) {
            setStep('part1');
          }
        }
      })
  ...
  const handleCheck = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) {
        setError(data.error ?? 'Fehler beim Prüfen');
        setStep(data.clickVerified ? 'part1_done' : 'part1');
        return;
      }
      if (data.expired) {
        setStep('expired');
        ...
        if (data.clickVerified) {
          setStep('part1_done');
        } else {
          setStep('part1');
        }
        setError(data.message ?? 'Noch nicht verifiziert.');
      }
    } catch {
      setError('Netzwerkfehler. Bitte erneut versuchen.');
      setStep('part1_done');
      ...
      <div className="space-y-4 text-white">
        ...
        <div className="flex items-center gap-2 text-xs">
          <StepBadge
            num={1}
            label="DM-Link klicken"
            done={['part1_done', 'checking', 'success'].includes(step)}
            active={['part1', 'starting'].includes(step)}
          />
          <div className="flex-1 h-px bg-zinc-700" />
          <StepBadge
            num={2}
            ...
            active={['part1_done', 'checking'].includes(step)}
          />
        </div>
        ...
        {step === 'idle' && (
          <div className="space-y-3">
            <p className="text-sm text-zinc-400">
              Du erhältst eine Instagram DM mit einem personalisierten Link.
              Klicke den Link (Teil 1) und teile dann den Beitrag in deiner Story (Teil 2) um die Belohnung zu erhalten.
            </p>
            <button
              onClick={handleStart}
              disabled={loading}
        ...
        {step === 'starting' && (
          <div className="text-center py-4">
            <div className="animate-spin w-8 h-8 border-2 border-pink-500 border-t-transparent rounded-full mx-auto mb-3" />
            <p className="text-sm text-zinc-400">DM wird gesendet…</p>
          </div>
        )}

        {/* ── PART 1: Warten auf Klick ── */}
        {step === 'part1' && (
          <div className="space-y-3">
            {dmSent ? (
              <div className="bg-green-900/30 border border-green-600/40 rounded-xl px-3 py-2 text-xs text-green-300 flex items-center gap-2">
                <FaCheck size={10} /> DM wurde an @{instagramHandle} gesendet
              </div>
            ) : (
              <div className="bg-zinc-800/60 border border-zinc-700 rounded-xl px-3 py-2 text-xs text-zinc-400">
                DM konnte nicht gesendet werden. Nutze den Link direkt:
              </div>
            )}
            ...
            {clickUrl && (
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="flex items-center justify-between gap-2 bg-zinc-800/60 border border-zinc-700 hover:border-pink-600/60 rounded-xl px-3 py-2 text-xs text-pink-300 transition-colors"
              >
            ...
            <div className="bg-zinc-800/60 rounded-xl px-3 py-3 text-sm text-zinc-300 space-y-1">
              <p className="font-semibold text-white text-xs">📩 Teil 1 – Link klicken</p>
              <p className="text-xs text-zinc-400">Klicke den Link in deiner DM. Der Klick wird automatisch registriert.</p>
            </div>

            {expiresAt && (
              <p className="text-center text-xs text-zinc-500">
                Verläuft in {formatTime(secondsLeft)}
              </p>
            )}
            ...
            <button
              onClick={handleCheck}
              disabled={loading}
              className="w-full bg-zinc-700 hover:bg-zinc-600 disabled:opacity-40 text-white font-semibold py-2.5 rounded-xl text-sm transition-colors flex items-center justify-center gap-2"
            >
              <FaRedo size={12} className={loading ? 'animate-spin' : ''} />
              Klick prüfen
            </button>
        ...
        {/* ── PART 1 DONE: Story teilen ── */}
        {(step === 'part1_done' || step === 'checking') && (
          <div className="space-y-3">
            <div className="bg-green-900/30 border border-green-600/40 rounded-xl px-3 py-2 text-xs text-green-300 flex items-center gap-2">
              <FaCheck size={10} /> Teil 1 bestätigt – Link wurde geklickt!
            </div>

            <div className="bg-zinc-800/60 rounded-xl px-3 py-3 space-y-1">
              <p className="font-semibold text-white text-xs">📸 Teil 2 – Story teilen</p>
              <p className="text-xs text-zinc-400">
                Gehe zu Instagram, öffne den Beitrag und teile ihn in deiner Story. Komm dann zurück und klicke &quot;Story prüfen&quot;.
              </p>
            </div>
            ...
            >
              <FaRedo size={12} className={loading ? 'animate-spin' : ''} />
              {loading ? 'Prüfe Story…' : 'Story prüfen'}
            </button>
          </div>
        )}
        ...
        {step === 'success' && (
          <div className="text-center py-4 space-y-3">
            ...
            <p className="font-bold text-lg text-green-400">Quest abgeschlossen!</p>
            <p className="text-sm text-zinc-400">
              +{rewardAmount} DFAITH Credits wurden gutgeschrieben.
            </p>
            <button onClick={onClose} className="w-full bg-zinc-700 hover:bg-zinc-600 text-white font-semibold py-2.5 rounded-xl text-sm transition-colors">
              Schließen
            </button>
