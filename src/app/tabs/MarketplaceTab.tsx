...
import { FaTag, FaTimes, FaCheckCircle, FaExclamationTriangle, FaGem, FaPlus, FaStar, FaCoins, FaMusic, FaSortAmountUp, FaSortAmountDown, FaClock, FaSlidersH, FaCreditCard, FaGift, FaCopy, FaInstagram, FaTiktok, FaFacebook, FaYoutube } from 'react-icons/fa';
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        // heliusMap.size > 0 bedeutet: Helius hat geantwortet → als On-Chain-Wahrheit verwenden
...
export default function MarketplaceTab() {
  ...
  const [view, setView]               = useState<'browse' | 'my' | 'giveaways'>('browse');
  ...
  return (
    <div className="w-full flex flex-col min-h-screen bg-[#0e0c0a] text-white pb-24">
      <div className="max-w-2xl mx-auto w-full">
        ...
        <div className="mx-4 mb-4">
          <div className="flex bg-white/[0.04] border border-white/[0.07] rounded-2xl p-1">
            ...
            {isArtist && (
              ...
              >
                <FaGift size={12} /> Giveaways
              </button>
            )}
          </div>
        </div>

        {/* ── Giveaways (nur Artists) ──────────────────────────────────────────── */}
        {view === 'giveaways' && isArtist && <GiveawaysPanel artistWallet={walletAddress} />}
        ...
        {/* ── Listings ────────────────────────────────────────────────────────── */}
        {view !== 'giveaways' && <div className="px-4">
          {loading && view === 'browse' ? (
