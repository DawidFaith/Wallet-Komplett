...
export default function LandingPage() {
  ...
  return (
    <main className="bg-[#0a0908] text-white min-h-screen">
      ...
      {/* ── SEKTION 1: Vollbild-Hero ── */}
      <div className="hidden lg:block relative" style={{ height: 'calc(100svh - 3.5rem)', marginTop: '3.5rem' }}>
        <Image
          src="/Still%202025-03-19%20193121_19.7.1.jpg"
          alt="Dawid Faith"
          fill
          ...
          priority
        />
        {/* Gradient unten für Textlesbarkeit */}
        <div className="absolute inset-0 bg-gradient-to-t from-[#0a0908] via-[#0a0908]/40 to-transparent" />
        {/* Leichtes Vignette rundum */}
        <div className="absolute inset-0 bg-gradient-to-r from-[#0a0908]/20 via-transparent to-[#0a0908]/20" />

        {/* Overlay-Content */}
        <div className="absolute bottom-0 left-0 right-0 px-20 pb-16">
          <div className="max-w-7xl mx-auto">
            {stats.openQuests > 0 && (
              <div className="flex items-center gap-2 mb-6">
                <FaFire size={12} className="text-amber-400" />
                <span className="text-[11px] font-black tracking-[0.25em] uppercase text-amber-400">
                  {stats.openQuests} {stats.openQuests === 1 ? 'Quest' : 'Quests'} verfügbar
                </span>
              </div>
            )}

            <h1 className="text-[5.5rem] font-black leading-[0.9] tracking-tight mb-7">
              Sei dabei.<br />
              Supporte deine<br />
              <span className="text-amber-400">Künstler.</span><br />
              Werde belohnt.
            ...
            <p className="text-base text-zinc-400 leading-relaxed mb-1 max-w-md">
              Das D.FAITH Ecosystem verbindet Künstler und Fans — mit echten Belohnungen und echter Community.
            </p>
            ...
            <div className="flex gap-4">
              <SignUpButton mode="modal">
                <button className="px-10 py-4 bg-amber-400 hover:bg-amber-300 active:scale-[0.98] text-black font-black text-sm tracking-[0.1em] uppercase rounded-2xl transition-all shadow-[0_0_60px_rgba(251,191,36,0.2)]">
                  Jetzt Supporter werden
                </button>
              </SignUpButton>
              <SignInButton mode="modal">
                <button className="px-8 py-4 border border-white/[0.08] hover:border-amber-400/20 text-zinc-500 hover:text-zinc-300 font-semibold text-[12px] tracking-[0.2em] uppercase transition-all rounded-2xl">
                  Bereits registriert? Einloggen
                </button>
              </SignInButton>
            </div>
        ...
        {/* Scroll-Hinweis */}
        <div className="absolute bottom-8 right-20 flex flex-col items-center gap-1.5 opacity-30">
          <span className="text-[9px] font-bold tracking-[0.3em] uppercase text-white">Scroll</span>
          <div className="w-px h-8 bg-white/40" />
        </div>
      </div>

      {/* ── SEKTION 2: Features (scrollbar darunter) ── */}
      <div className="hidden lg:block bg-[#0a0908] border-t border-white/[0.04]">
        <div className="max-w-7xl mx-auto px-20 py-24">

          {/* Heading */}
          <div className="mb-16">
            <p className="text-[11px] font-black tracking-[0.4em] uppercase text-zinc-700 mb-3">
              Web3 · Musik · Community
            </p>
            <h2 className="text-[3rem] font-black text-white leading-tight">
              Werde Teil der <span className="text-amber-400">Bewegung.</span>
            </h2>
          </div>

          {/* Tab-Umschalter */}
          <div className="flex gap-10 border-b border-white/[0.07] mb-14">
            {(['fan', 'artist'] as const).map((t) => (
              <button
                key={t}
                onClick={() => setTab(t)}
                className={`pb-4 text-[13px] font-black tracking-[0.25em] uppercase border-b-2 transition-all -mb-px ${
                  tab === t
                    ? 'border-amber-400 text-white'
                    : 'border-transparent text-zinc-600 hover:text-zinc-400'
                }`}
          ...
          {tab === 'fan' ? (
            <div className="grid grid-cols-2 gap-x-20 gap-y-10">
              {fanFeatures.map((f) => (
                <div key={f.n} className="flex gap-6 items-start">
                  <span className="text-[13px] font-black text-amber-400/30 w-8 shrink-0 pt-0.5">{f.n}</span>
                  <div>
                    <p className="text-[1.1rem] font-bold text-white mb-2">{f.title}</p>
              ...
              {!applied ? (
                <div className="grid grid-cols-2 gap-x-20 gap-y-10">
                  ...
                  <div className="space-y-4 max-w-md">
                    <div>
                      <label className="text-[10px] font-black tracking-[0.3em] uppercase text-zinc-600 mb-2 block">
                        Künstlername *
                      </label>
                      <input
                        value={artistName}
                        onChange={(e) => setArtistName(e.target.value)}
                        placeholder="Dein Name"
                        className="w-full bg-white/[0.04] border border-white/[0.07] focus:border-amber-400/30 rounded-xl px-5 py-4 text-sm text-white placeholder-zinc-700 outline-none transition-colors"
                      />
                    </div>
                    <div>
                      <label className="text-[10px] font-black tracking-[0.3em] uppercase text-zinc-600 mb-2 block">
                    ...
                    >
                      {sending ? 'Sende…' : <>Interesse anmelden <FaChevronRight size={10} /></>}
                    </button>
                  </div>
                </div>
              ) : (
                <div className="py-20 text-center">
                  <FaCheckCircle size={40} className="text-amber-400 mx-auto mb-5" />
                  <p className="text-2xl font-black text-white mb-3">Danke, {artistName}!</p>
                  <p className="text-zinc-500">Wir melden uns so schnell wie möglich.</p>
                </div>
              )}
            </div>
          )}
